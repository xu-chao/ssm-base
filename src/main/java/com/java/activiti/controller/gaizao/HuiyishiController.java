...
public class HuiyishiController {
    private static final int HYS_INCREMENT_LENGTH = 10;//流水号长度
    private static final int HYS_DBINDEX = 9;//redisDB缓存区
    private static final String HYS_ID = "HYS_ID";//redisDB缓存区
...
//        String huiyishiID = UuidUtil.uuidUtil();
        ...
        if (resultTotal > 0) {
            ...
            variables.put("hys2", huiyishiAdviser);//审批人
            variables.put("hys1", UserUtil.getSubjectUserID());//提交人
            // 启动流程
            ProcessInstance pi = runtimeService.startProcessInstanceByKey("huiyishi", variables);
            // 根据流程实例Id查询任务
            Task task = taskService.createTaskQuery().processInstanceId(pi.getProcessInstanceId()).singleResult();
            ...
            //修改状态
            huiyishiIn.setState("审核中");
            huiyishiIn.setStateID(1);//审核中

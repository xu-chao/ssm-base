...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/swylsbJh/findById",
                type: "post",
                data: {"id": 3},//返回给客户端的json数据
                async: false,
                dataType: 'json',
                success: function (data) {
                    var swylsbJb = data.total;
                    for(var item in swylsbJb) {
                        if (swylsbJb[item].jhStartDate != null) {
                            $('#2'+swylsbJb[item].jdField+'-jhStartDate').datebox('setValue', new Date(swylsbJb[item].jhStartDate.time).format("Y-m-d"));
                        }
                        if (swylsbJb[item].jhEndDate != null) {
                            $('#2'+swylsbJb[item].jdField+'-jhEndDate').datebox('setValue', new Date(swylsbJb[item].jhEndDate.time).format("Y-m-d"));
                        } if (swylsbJb[item].sjStartDate != null) {
                            $('#2'+swylsbJb[item].jdField+'-sjStartDate').datebox('setValue', new Date(swylsbJb[item].sjStartDate.time).format("Y-m-d"));
                        }
                        if (swylsbJb[item].sjEndDate != null) {
                            $('#2' + swylsbJb[item].jdField + '-sjEndDate').datebox('setValue', new Date(swylsbJb[item].sjEndDate.time).format("Y-m-d"));
                        }
                    }
                }
            });
                        ...
                        <td>
                            <table class="TailTable">
                                <tr>
                                    ...
                                    <input id="sbztXq" name="sbzt" type="hidden"/>
                                </tr>
                                <tr>

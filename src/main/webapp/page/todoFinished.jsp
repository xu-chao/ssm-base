    ...
    <script type="text/javascript">
        ...
        $(function () {
            $.post("${pageContext.request.contextPath}/repair/getRepairByTaskId", {taskId: '${param.taskId}'}, function (result) {
                ...
                workFileID = workOrder.repairedFileID;
                ...
                $("#woStartDate").text(workOrder.woStartDate);
                $("#woType").text(workOrder.woType);
                $("#materielID").text(workOrder.materielID);
                $("#materielType").text(workOrder.materielType);
                $("#repairedHelper").text(workOrder.repairedHelper);
                $("#repairedProcess").text(workOrder.repairedProcess);
            }, "json");
        });
        ...
        function openUpFile(fileID) {
            ...
            $('#findFiles').datagrid({
                ...
                striped: true,
                nowrap: false,
                url: '${pageContext.request.contextPath}/fileInfo/findFiles?fileID=' + fileID,
                columns: [[
        ...
        function editRow(path) {
            ...
            form.attr("action","${pageContext.request.contextPath}/fileInfo/downloadFile");// 文件下载提交路径
            var input1=$("<input>");
        ...
        <form id="ff" method="post">
            <table class="table table-border table-bordered table-striped" style="width: 100%" title="故障反馈单">
                ...
                <tr>
                    ...
                    <td>物料名称：</td>
                    <td>
                        <span id="materielID"></span>
                    </td>
                </tr>
                <tr>
                    <td>物料类型：</td>
                    <td>
                        <span id="materielType"></span>
                    </td>
                    <td>需要维修协助人：</td>
                    <td>
                        <span id="repairedHelper"></span>
                    </td>
                </tr>

                <tr>
                    <td valign="top">维修过程：</td>
                    <td colspan="4">
                        <span id="repairedProcess" rows="2" cols="50"></span>
                    </td>
                </tr>
                <tr>
                    <td valign="top">文件浏览：</td>
                    <td colspan="4">
                        <a class="easyui-linkbutton" onclick="openUpFile(repairFileID);">附件-现场图片</a>
                        <a class="easyui-linkbutton" onclick="openUpFile(workFileID);">附件-反馈图片</a>
                    </td>
                </tr>
                <tr>
                    <td>批注：</td>

...
<script type="text/javascript">
	function deleteGroup(){
		...
		$.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectRows.length+"</font>条数据吗?",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/group/deleteGroup",{ids:ids},function(result){
					if(result.success){
	...
	function checkData(){
		...
		var flag=$("#flag").val();
		if(flag==1){
			$.post("${pageContext.request.contextPath}/group/existGroupName",{groupID:$("#id").val()},function(result){
				if(result.success){
	...
	function saveGroup(){
		$("#fm").form("submit",{
			url:'${pageContext.request.contextPath}/group/groupSave',
			onSubmit:function(){
 	...
 	<form id="fm" method="post">
 		<table cellpadding="8px">
 			<tr>
 				<td>角色名：</td>
 				<td>
 					<input type="text" id="id" name="id" class="easyui-validatebox" required="true" readonly="true"/>
 				</td>
 				<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
 				<td>角色名称：</td>
 				<td>

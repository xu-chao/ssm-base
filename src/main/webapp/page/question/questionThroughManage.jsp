...
<head>
    ...
    <script type="text/javascript">
        ...
        var root = "${pageContext.request.contextPath}/question/";
        var sessionFile = "${sessionFile}";
        var pageName = 'questionThrough';
        var name = 'Question';
        var title = '问题列表';
        var idField = 'questionID';
        ...
        var columns = [[{
            ...
            align: 'center',
            formatter: function (value, row, index) {
                var btn = '<a class="editcls" onclick="openFiles(\'' + row.fileBeforeID + '\')" href="javascript:void(0)">反馈文件</a>&nbsp;&nbsp;' +
                    '&nbsp;<a href="javascript:openListCommentDialog(\'' + row.processInstanceId + '\')">历史批注</a>&nbsp;' +
                    '&nbsp;&nbsp;<a class="editcls" onclick="downWord(\'' + row.processInstanceId + '\',\'' + row.questionID + '\')" href="javascript:void(0)">文件ZIP</a>';
                return btn;
            }
        }]];
        ...
        $(document).ready(function () {
            ...
            $('#dg2').datagrid({
                fitColumns: true,
                url: "${pageContext.request.contextPath}/questionTask/listHistoryCommentWithProcessInstanceId",
                columns: [[
        ...
        function downWord(processId, questionID) {
            ...
            form.attr("target", "_blank");
            form.attr("method", "post");
            form.attr("action", "${pageContext.request.contextPath}/zipFile/getDownFileZip");    // 此处填写文件下载提交路径
            var input1 = $("<input>");
            input1.attr("type", "hidden");
            input1.attr("name", "questionID");    // 后台接收参数名
            input1.attr("value", questionID);
            $("body").append(form);    // 将表单放置在web中
            form.append(input1);
            form.submit();    // 表单提交
        }

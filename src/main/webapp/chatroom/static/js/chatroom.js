    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 200) {
                ...
                userChat = data.data.userChat;
                userId = userChat.userId;
                setSentMessageMap();//初始化前端消息对象
                console.info(userChat);
                $("#username").html(userChat.username);
                $("#avatarUrl").attr("src", userChat.avatarUrl);
                var groupListHTML = "";
                var groupList = userChat.groupList;
                for (var i = 0; i < groupList.length; i++) {//userChat.groupList;对象没有初始化
                    groupListHTML +=
                        '<li>' +
                ...
                var friendListHTML = "";
                var friendList = userChat.friendList;
                for (var i = 0; i < friendList.length; i++) {
                    friendListHTML +=
                        '<li>' +
                        '<div class="liLeft"><img src="' + friendList[i].userChat2.avatarUrl + '"></div>' +
                        '<div class="liRight">' +
                        '<span class="hidden-userId">' + friendList[i].userChat2.userId + '</span>' +
                        '<span class="intername">' + friendList[i].userChat2.userName + '</span>' +
                        '<span class="infor"></span>' +
                        '</div>' +
                        '</li>';
                }
...
function setHistoryMessage() {
    var MessageList=userChat.messageList;//这是历史单体消息填充
    for(var i=0;i<MessageList.length;i++){
        switchMessage(MessageList[i]);
    }
    var GroupList=userChat.groupList;//这是历史群组消息填充
...
function setSentMessageMap() {
    ...
    var friendList = userChat.friendList;
    for (var i = 0; i < friendList.length; i++) {
        sentMessageMap.put(friendList[i].userChat2.userId, new Array());
    }
    var groupList = userChat.groupList;
    for (var i = 0; i < groupList.length; i++) {
